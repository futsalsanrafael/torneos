...
with tab1:
    try:
        with open(f'{root_path}/data/a3.json', 'r') as jfile:
        ...
        st.error(f"Error parsing a3.json: {str(e)}")
        st.stop()
    except FileNotFoundError:
        st.error("Error: a3.json not found in the data directory.")
        st.stop()
...
with tab3:
    # Load a3-statistics.csv
    try:
        csv_path = f'{root_path}/data/a3-statistics.csv'
        df = pd.read_csv(csv_path)
    except FileNotFoundError:
        logging.warning("Error: a3-statistics.csv not found in the data directory.")
        st.header("Tabla de goleadores aún no disponible.")
        st.stop()
    except pd.errors.EmptyDataError:
        st.error("Error: a3-statistics.csv is empty.")
        st.stop()
    except pd.errors.ParserError:
        st.error("Error: Invalid CSV format in a3-statistics.csv.")
        st.stop()
