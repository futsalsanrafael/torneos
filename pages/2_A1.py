...
with tab3:
    ...
    try:
        csv_path = f'{root_path}/data/a1-statistics.csv'
        df = pd.read_csv(csv_path)
    except FileNotFoundError:
        logging.warning("Error: a1-statistics.csv not found in the data directory.")
        st.header("Tabla de goleadores aún no disponible.")
        st.stop()
    except pd.errors.EmptyDataError:
        st.error("Error: a1-statistics.csv is empty.")
        st.stop()
    except pd.errors.ParserError:
        st.error("Error: Invalid CSV format in a1-statistics.csv.")
        st.stop()
