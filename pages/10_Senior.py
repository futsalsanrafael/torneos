...
st.markdown(body="# Senior", width="content")
...
with tab1:
    try:
        with open(f'{root_path}/data/senior.json', 'r') as jfile:
        ...
        st.error(f"Error parsing senior.json: {str(e)}")
        st.stop()
    except FileNotFoundError:
        st.error("Error: senior.json not found in the data directory.")
        st.stop()
...
with tab3:
    # Load senior-statistics.csv
    try:
        csv_path = f'{root_path}/data/senior-statistics.csv'
        df = pd.read_csv(csv_path)
    except FileNotFoundError:
        logging.warning("Error: senior-statistics.csv not found in the data directory.")
        st.header("Tabla de goleadores aún no disponible.")
        st.stop()
    except pd.errors.EmptyDataError:
        st.error("Error: senior-statistics.csv is empty.")
        st.stop()
    except pd.errors.ParserError:
        st.error("Error: Invalid CSV format in senior-statistics.csv.")
        st.stop()
