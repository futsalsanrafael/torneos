...
# Femenino page content
st.set_page_config(
    page_title="Futsal De Toque",
    page_icon=":material/sports_soccer:",
    layout="wide"
)
st.markdown(body="# Femenino", width="content")
...
with tab1:
    try:
        with open(f'{root_path}/data/femenino.json', 'r') as jfile:
        ...
        st.error(f"Error parsing femenino.json: {str(e)}")
        st.stop()
    except FileNotFoundError:
        st.error("Error: femenino.json not found in the data directory.")
        st.stop()
...
with tab3:
    # Load femenino-statistics.csv
    try:
        csv_path = f'{root_path}/data/femenino-statistics.csv'
        df = pd.read_csv(csv_path)
    except FileNotFoundError:
        logging.warning("Error: femenino-statistics.csv not found in the data directory.")
        st.header("Tabla de goleadores aún no disponible.")
        st.stop()
    except pd.errors.EmptyDataError:
        st.error("Error: femenino-statistics.csv is empty.")
        st.stop()
    except pd.errors.ParserError:
        st.error("Error: Invalid CSV format in femenino-statistics.csv.")
        st.stop()
